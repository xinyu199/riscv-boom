  ...
  with HasBoomFrontendParameters // TODO: Don't add this trait
{
  ...
  //recorder
  val record_setting  = RegInit(0x00F01010.U(32.W))
  val record_thrInit  = record_setting(7, 0)    //16
  val record_thrStep  = record_setting(15, 8)   //16
  val record_thrMaxv  = record_setting(23, 16)  //240
  val record_entSel   = record_setting(31, 24)  //0

  var event_recorder: EventRecoders_SRAM1 = null
  if(hasRecorder) {
    event_recorder = Module(new EventRecoders_SRAM1(exe_units.numIrfReaders))
    event_recorder.io.thrInit   := record_thrInit
    event_recorder.io.thrStep   := record_thrStep
    event_recorder.io.thrMaxv   := record_thrMaxv

    event_recorder.io.event_val := false.B
    event_recorder.io.reset_recorder := false.B
    event_recorder.io.event_val := false.B
    event_recorder.io.event_pc  := 0.U
  }
...
//      ("I$ blocked",                        () => icache_blocked),
  ...
  if(hasRecorder) {
    //start read recorder
    for (w <- 0 until exe_units.numIrfReaders) {
      event_recorder.io.read_addr(w).valid := iss_valids(w) && iss_uops(w).readRecorder && iss_uops(w).ldst =/= 0.U
      event_recorder.io.read_addr(w).bits := iss_uops(w).inst(29, 20)
    }
    ...
    for (w <- 0 until coreWidth) {
      val uop = rob.io.commit.uops(w)
      when (rob.io.commit.valids(w) && uop.readRecorder && uop.inst(30, 20) === 2047.U) { 
        event_recorder.io.reset_recorder := true.B
      }
    }
  }
  ...
  for(w <- 0 until coreWidth) {
    ...
    ldq_dis_stall(w) := io.lsu.ldq_full(w) && dis_uops(w).uses_ldq && dis_valids(w)
    stq_dis_stall(w) := io.lsu.stq_full(w) && dis_uops(w).uses_stq && dis_valids(w)
  }
  ...
  //topdown
  //-- first level
  val before_misbp = RegInit(false.B)
  val before_flush = RegInit(false.B)
  val has_valid_dec = dec_fire.reduce(_||_)
  ...
  when (has_valid_dec) {
    before_misbp := false.B
    before_flush := false.B
  }

  //--- idq_uops_not_delivered.core
  //counts the number of uops not delivered from IDQ to RAT and RAT is not stalled
  val dec_uops_not_delivered = Wire(Vec(coreWidth, Bool()))
  for(w <- 0 until coreWidth) {
    val back_stall = !dis_ready && branch_mask_full(w)
    dec_uops_not_delivered(w) := !dec_valids(w) && !before_misbp && !before_flush && !back_stall
  }

  //int_misc.recovery_cycles
  val is_misc_recoverying = WireInit(false.B)
  is_misc_recoverying := rob.io.commit.rollback || before_misbp || before_flush

  //--- uops_issued.any
  val iss_int_uops = Wire(Vec(exe_units.numIrfReaders, Bool()))
  for(w <- 0 until exe_units.numIrfReaders) {
  ...
  //--- fetch latency	: FetchBubbles[≥ #MIW] / Clocks | idq_uops_not_delivered.core: [≥ 4] / clocks
  //Machine Issue Width (MIW)
  val is_fetch_lat_cause_cycle = WireInit(false.B)
  is_fetch_lat_cause_cycle := dec_uops_not_delivered.reduce(_&&_) //全部为1才算是

  //--- cycle_activity.cycles_no_execute 
  val exe_int_valids = Wire(Vec(exe_units.length, Bool()))
  val exe_uops_num = Wire(UInt(4.W))
  val is_no_exe_cycle = WireInit(false.B)
  val is_one_exe_cycle = WireInit(false.B)
  for (w <- 0 until exe_units.length) {
    exe_int_valids(w) := exe_units(w).io.req.valid
  }
  exe_uops_num := PopCount(exe_int_valids) + fp_pipeline.io.exe_fp_uops
  is_no_exe_cycle := (exe_uops_num === 0.U)
  is_one_exe_cycle := (exe_uops_num === 1.U)

  val is_iq_empty = WireInit(false.B)
  is_iq_empty := mem_iss_unit.io.event_empty && int_iss_unit.io.event_empty && fp_pipeline.io.fp_iq_event_empty

  //--- MemStalls.AnyLoad
  //--- MemStalls.Stores
  val is_memstall_anyload = WireInit(false.B)
  val is_memstall_stores = WireInit(false.B)
  ...
  when (RegNext(rob.io.flush.valid)) {
    ...
    if(hasRecorder) {
      // val isRecorded = true.B 
      val isRecorded = csr.io.status.prv === 0.U && isTargetProc
      when (record_entSel === RcdEvent_AllMisBranch) {
        event_recorder.io.event_val := isRecorded
        event_recorder.io.event_pc  := uop_maybe_pc(32, 1)
      }
      .elsewhen (record_entSel === RcdEvent_MisBr) {
        when (brupdate.b2.cfi_type === CFI_BR) {
          event_recorder.io.event_val := isRecorded
          event_recorder.io.event_pc  := uop_maybe_pc(32, 1)
        }
      }
      .elsewhen (record_entSel === RcdEvent_MisJr) {
        when (brupdate.b2.cfi_type === CFI_JALR) {
          event_recorder.io.event_val := isRecorded
          event_recorder.io.event_pc  := uop_maybe_pc(32, 1)
        }
      }
    ...
    printf("redirect core by mispred, inst pc: 0x%x \n", uop_maybe_pc)
...
//   "CSRFile is being written to spuriously.")
  ...
  for (w <- 0 until exe_units.length) {
    ...
    if (exe_unit.readsIrf) {
      ...
      if(hasRecorder) {
        when (uop.readRecorder && uop.ldst =/= 0.U) {
          exe_unit.io.req.bits.rs1_data := event_recorder.io.read_data(iss_idx)
        }
      }
